...
export default function DatabaseTableView(props: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper className={clsx(selected ? "ProseMirror-selectednode" : "")}>
      ...
      {data && (() => {
        // Gruppierung aus Baserow-View: gleiche Werte in der Group-By-Spalte werden
        // zeilenübergreifend als EINE Zelle dargestellt (rowspan, wie Excel-Zellverbund).
              ...
              <Table.Tbody>
                {data.rows.map((row, ri) => (
                  <Table.Tr key={ri}>
                    ...
                    {row.map((cell, ci) => {
                      // Gruppen-Spalte: Folgezeilen überspringen, Erstzelle mit rowSpan
